...
/datum/species/proc/spec_attacked_by(obj/item/I, mob/living/user, obj/item/bodypart/affecting, intent, mob/living/carbon/human/H)
	...
	var/bloody = FALSE
	if(((I.damtype == BRUTE) && I.force && prob(30 + (I.force * 2))))
		...
		switch(affecting.body_zone)
			if(BODY_ZONE_HEAD)
				if(armor_block < 40) //Арморблок снижен на 10 по просьбе Псины.
					if(prob(I.force))
						...
						if(prob(10)) //Балансим шанс получить травму от двойного шанса на нокаут. (Дефолт=10)
				...
				else if(armor_block < 45)
					if(prob(I.force))
						H.adjustOrganLoss(ORGAN_SLOT_BRAIN, I.force * 0.6)
						if(H.stat == CONSCIOUS)
							H.visible_message(span_danger("[H] хочет прилечь!") , \
											span_userdanger("Теряю сознание!"))
							H.apply_effect(195, EFFECT_UNCONSCIOUS)
						if(prob(10))
							H.gain_trauma(/datum/brain_trauma/severe/narcolepsy)
					else
						H.adjustOrganLoss(ORGAN_SLOT_BRAIN, I.force * 0.3)
			...
			if(BODY_ZONE_CHEST)
				if(H.stat == CONSCIOUS && armor_block < 50)
					if(prob(I.force))
						...
						H.apply_effect(70, EFFECT_KNOCKDOWN, armor_block)
						H.apply_effect(50, EFFECT_STUN, armor_block)

				else if(H.stat == CONSCIOUS && armor_block < 60)
					if(prob(I.force))
						H.visible_message(span_danger("[H] парализован[H.ru_a()]!") , \
									span_userdanger("Меня парализовало!"))
						H.apply_effect(190, EFFECT_PARALYZE, armor_block)
						H.adjustOxyLoss(30)
