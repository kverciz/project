	...
	if(limb.body_zone == BODY_ZONE_HEAD)
		...
		switch(blood_bled)
			if(1 to 6)
				victim.bleed(blood_bled, TRUE)
			if(7 to 13)
				victim.visible_message(span_danger("Кровь начинает бежать из головы [victim]!") , span_danger("Чувствую кровушка побежала по головушке.") , vision_distance=COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled, TRUE)
				if(prob(30))
					brain.applyOrganDamage(25)
			if(14 to 19)
				victim.visible_message(span_smalldanger("Голова [victim] слегка брызнула кровью!") , span_danger("Сколько крови вылетело из моей головы!") , vision_distance=COMBAT_MESSAGE_RANGE)
				new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
				victim.bleed(blood_bled)
				if(prob(40))
					brain.applyOrganDamage(35)
			if(20 to INFINITY)
				victim.visible_message(span_danger("Голова [victim] брызжет кровью!") , span_danger("<b>МОЯ ГОЛОВА! КРОВЬ!</b>") , vision_distance=COMBAT_MESSAGE_RANGE)
				victim.bleed(blood_bled)
				new /obj/effect/temp_visual/dir_setting/bloodsplatter(victim.loc, victim.dir)
				victim.add_splatter_floor(get_step(victim.loc, victim.dir))
				if(prob(40))
...
/datum/wound/blunt/critical/apply_wound(obj/item/bodypart/L, silent, datum/wound/old_wound, smited)
	if(L.body_zone == BODY_ZONE_HEAD)
		occur_text = "хрустит, обнажая треснувший череп сквозь плоть и кровь"
